 ...
 * @author Yassine
 */
public class ListRecetteCoachForm extends BaseForm{
    Resources res;
    public ListRecetteCoachForm(Form previous) throws IOException {
        ...
        ArrayList<Recette> ALR = ServiceRecette.getInstance().getAllRecettesCoach();
        ...
        page.addAll(tfRecherche,btnRecherche,hi);
           btnRecherche.addPointerPressedListener(e->{
               if(tfRecherche.getText().length()!=0){
                    System.out.println(ServiceRecette.getInstance().ReadRecette(tfRecherche.getText()));
